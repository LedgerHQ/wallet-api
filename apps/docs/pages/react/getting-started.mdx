<div>
  ...
  **Please Note:** The `@ledgerhq/wallet-api-simulator` package currently only functions with server-side applications.
  This is due to a dependency requiring `process`, which is polyfilled inside Server Side Rendering (SSR) frameworks
  like Next.js or Remix. We are actively working to remove that dependency and will update this guide once it is available.

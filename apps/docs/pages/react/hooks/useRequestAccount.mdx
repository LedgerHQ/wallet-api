...
The `useRequestAccount` hook facilitates requesting access to a user’s account. It is important to note that on component mount, the user will be prompted to select an account from the Ledger Wallet.
...
The `requestAccount` function does not take any parameters.

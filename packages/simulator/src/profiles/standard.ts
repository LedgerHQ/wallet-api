...
export function setProfile(serverInstance: WalletAPIServer) {
  ...
  serverInstance.setHandler("transaction.signAndBroadcast", async () => {
    return "0xtxHash";
  });

  serverInstance.setHandler("transaction.sign", async () => {
    return Buffer.from(
      "0x123O182493423928734983247923847293847293847923847293487"
    );
  });

  serverInstance.setHandler("message.sign", async () => {
    return Buffer.from("0x123456789123456789");
  });

  serverInstance.setHandler("account.receive", async () => {
    return "eth address";
  });
}

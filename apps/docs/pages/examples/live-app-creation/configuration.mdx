...
During the development and testing phases, it's beneficial to use the Ledger Wallet API simulator instead of the real Ledger Live environment. Modify your `TransportProvider.tsx` file to use the `getSimulatorTransport` function provided by the simulator library.
